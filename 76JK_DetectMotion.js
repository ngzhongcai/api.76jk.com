...
exports.handler= function(event, context, callback) {
    ...
    verifyToken(event, function(err, res) {
        ...
        if(err) { context.fail("501::76JK_DETECT_MOTION::VERIFY_TOKEN::" + err.toString()); return; }
        publishToDisplay(event, function(err, res) {
            ...
            if(err) { context.fail("501::76JK_DETECT_MOTION::PUBLISH_TO_DISPLAY::" + err.toString()); return; }
...
const verifyToken= function(event, callback) {
    ...
    if(!authHeader.startsWith("Bearer ")) { callback("NOT_AUTHROIZED"); return; }
    const token= authHeader.substring("Bearer ".length);
    if(token!== process.env.SECRET) { callback("NOT_AUTHORIZED!"); return; }
    callback();
}

var publishToDisplay= function(event, callback) {
    var obj= {}; obj.action= "motion"; obj.view= "bonsai"; var payload= JSON.stringify(obj);
