  ...
  verify76JK(event, function(err, res) {
    ...
    getTagFromDynamo(event, function(err, res) {
      ...
      updateTagIntoDynamo(event, function(err, res) {
        ...
        deletePhotoFromS3(event, function(err, res) {
          now= Math.round(new Date().getTime()); timetaken= timetaken+ now- last; timings.push("DELETE_PHOTO_FROM_S3::" + (now- last)); last= now;
          if(err) { context.fail("504::76JK_DELETE_ENTRY::DELETE_PHOTO_FROM_S3::" + err.toString()); return; }
...
const deletePhotoFromS3= function(event, callback) {
}